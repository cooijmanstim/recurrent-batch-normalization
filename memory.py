...
def construct_lstm(args, x, activation):
    ...
    if args.baseline:
        ...
        def bn(x, gammas, betas):
            ...
            mean.tag.batchstat, var.tag.batchstat = True, True
            #var = T.maximum(var, args.epsilon)
            vareps = var + args.epsilon
            return (x - mean) / T.sqrt(vareps) * gammas + betas, mean, var
    ...
    def stepfn(xtilde, dummy_h, dummy_c, h, c):
        atilde = T.dot(h, Wa)
        btilde = xtilde
        a, mean, var = bn(atilde, a_gammas, ab_betas)
        ...
        c = dummy_c + f * c + i * g
        htilde = c
        bn_, tmp1, tmp2 = bn(htilde, h_gammas, h_betas)
        h = dummy_h + o * activation(bn_)
        return h, c, atilde, btilde, htilde, g, f, i, o, mean, var

    [h, c,
     atilde, btilde, htilde,
     g, f, i, o,
     mean, var], _ = theano.scan(
         stepfn,
         ...
         outputs_info=[T.repeat(h0[None, :], xtilde.shape[1], axis=0) + h_prime,
                       ...
                       None, None, None, None,
                       None, None])
    return dict(h=h, c=c, atilde=atilde, btilde=btilde, htilde=htilde,
                g=g, f=f, i=i, o=o,
                mean=mean, var=var), dummy_states, parameters
...
def construct_common_graph(situation, args, outputs, mask, dummy_states, Wy, by, y):
    ...
    cross_entropies = T.nnet.categorical_crossentropy(flat_yhat, flat_y).reshape(mask.shape)

    ## masking
    errors = mask * errors
    cross_entropies = mask * cross_entropies
...
def construct_graphs(args, nclasses, length):
    ...
    training_graph, training_extensions = construct_common_graph("training", args, outputs, mask, dummy_states, Wy, by, y)
    ...
    inference_graph, inference_extensions = construct_common_graph("inference", args, inf_outputs, mask, dummy_states, Wy, by, y)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-hidden", type=int, default=40)
    ...
    parser.add_argument("--clipvar", action="store_true")
    ...
    sequence_length = args.interval+20
        ...
        for which_set in "train valid test".split():
            ...
            extensions.append(DataStreamMonitoring(
                ...
                data_stream=get_stream(which_set=which_set,
                                       min_interval=args.interval, max_interval=args.interval,
                                       batch_size=args.batch_size)))#, num_examples=1000)))
