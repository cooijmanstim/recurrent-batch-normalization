...
On the other hand, previous works only apply batch normalization to the input-to-hidden transformation of RNNs,
...
it has beeb proven to be difficult to apply in recurrent architectures~\cite{cesar,baidu}.
It has found limited use in stacked RNNs, where the normalization is applied ``vertically'', 
i.e. to the input of each RNN, but not ``horizontally'' between time-steps.
...
%% FIXME add section 4
Section~\ref{sec:prerequisites}, introduces RNNs and batch normalization in details.
...
recurrent models. We show in Section~\ref{sec:experiments} that, our evaluations 
of the proposed re-parameterization on a variety of tasks.
...
Vanishing gradient problem occurs when hidden-states $h_t$ are not influenced by small changes in much earlier states $h_{\tau}$, $t \ll \tau$,
...
at each time-step, the update of the LSTM cell is regulated by a set of gates.
...
This section introduces a re-parameterization of LSTM that takes advantage of 
