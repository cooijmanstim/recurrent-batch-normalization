...
It is defined as the distributions shift of a layer input due to the change in network parameters.
Indeed, a layer in a network needs to continuously adapt to its input distribution which changes by updating the parameters of the layers below.
...
In deep neural networks, the distribution of activations presented to a layer $l$ is determined by the parameters of the layers below $l$.
As we update the network parameters during training, this distribution can vary, leading to \emph{internal covariate shift}~\cite{batchnorm}.
...
However, it does not decorrelate the activations as that involves matrix inversion which is too expensive.
