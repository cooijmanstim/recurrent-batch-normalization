...
\begin{tabular}{@{}lc@{}}
  ...
  BN-LSTM (ours) & 1.32 \\
  \midrule
  Zoneout~\citep{krueger2016zoneout} & 1.28 \\
  HM-LSTM~\citep{chung2016hierarchical} & 1.24 \\
  HyperNetworks~\citep{ha2016hypernetworks} & 1.22 \\
  \bottomrule
\end{tabular}
...
We observe that BN-LSTM obtains a significant performance improvement over the LSTM baseline. In addition, our BN-LSTM model achieves state of the art. %FIXME
...
After incorporating these improvements, our new results on the reduced task described in~\ref{sec:less-attr} are shown in Figure~\ref{fig:attr_valid2}.
...
baseline: 54.5/55.0
bn-bidir-120-units: 62.1/63.7 (120 units, lr8e-4 batch size 40)
deepmind: 61.6/63.0

shown in Figure~\ref{fig:attr_full_valid}.
