...
Recurent Neural Networks (RNN)  have recently exhibited state-of-art on a wide range of complex sequential problems including speech recognition~\cite{baidu}, machine translation~\cite{bahdanau2014neural} or image/video captionning~\cite{xu2015show,yao2015describing}. However, training RNN using first-order stochastic gradient descent (SGD) remains a difficult task due to well-known exploding/vanishing gradient problem~\cite{bengio1994learning,hochreiter1991untersuchungen,pascanudifficulty}.
In the second case, errors of a given state will not influence the states far in the past as the gradient vanish when being backpropagated through-time,
...
In section we develop the observation that high-variance pre-activations tend to saturate $\tanh$ nonlinearities.
