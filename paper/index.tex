...
This section presents an empirical evaluation of the proposed batch-normalized LSTM on five different tasks. Results show that batch-normalized LSTM achieves convergence faster than a baseline LSTM on all the tasks, and, can also lead to better generalization.
...
\begin{figure}
  ...
  \hspace{2mm}%
  \subfigure[
    Generalization to longer subsequences (of the Penn Treebank test sequence) using population statistics.
]{%
...
reparameterization, we propose to use batch normalized LSTM in
the Attentive Reader Model~\cite{attentivereader}.
We take the Attentive Reader model\footnote{we take an existing implementation that can be found at \url{https://github.com/caglar/Attentive_reader}}
 and simply replace the LSTM with our BN-LSTM reparametrization.
Additionally we try a variant, named BN-Eveywhere, where we also introduce batch
normalization into the attention computations, normalizing each term
going into the $\tanh$ nonlinearities.

%% FIXME model hyperparameters
Our Attentive Reader LSTM is composed by FIXME hiddent units
We use stochastic gradient descent on minibatches of size FIXME,
with gradient clipping at FIXME and step rule determined by Adam~\cite{kingma2014adam}
with learning rate FIXME.
We use orthogonal initialization for all weight matrices.
BN-LSTM and BN-everywhere relies on the exact same settings.
...
Figure~\ref{fig:attr_valid} reports the training and validation accuracies for the different variants of the attentive reader.
BN-LSTM achieves a dramatic acceleration in term of training convergence compared to the baseline LSTM.
BN-Everywhere, applying batch normalization in the attention computations as well, also shows a significant
improvement over BN-LSTM.
In addition, both BN-LSTM and BN-Everywhere show a generalization benefit, in term of validation accuracy, over the baseline.
%% FIXME should we add a table for that
