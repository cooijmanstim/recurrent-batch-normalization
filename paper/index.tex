...
Recurent Neural Networks (RNN)  have recently exhibited state-of-art on a wide range of complex sequential problems including speech recognition~\cite{baidu}, machine translation~\cite{bahdanau2014neural} or image/video captionning~\cite{xu2015show,yao2015describing}. However, training RNN using first-order stochastic gradient descent (SGD) remains a difficult task due to well-known vanishing gradient problem~\cite{bengio1994learning,hochreiter1991untersuchungen,pascanudifficulty}.
Errors of a given state will not influence the sates far in the past as the gradient vanish when being backpropagated through-time,
preventing RNN to capture long-term dependencies in the input data.
While the long-term dependencies problem is unsolvable in absolute~\cite{bengio1994learning}, several RNN reparametrization, such the LSTM~\cite{lstm}, GRU~\cite{cho2014learning} or $i$/$u$-RNN~\cite{le2015simple,urnn} have been found to help mitigate the vanishing gradient problem.
...
Investigating difference parametrization of RNN is might allows to find model that are easier to optimize and are able to capture the data long-term dependencies.
...
This leads to the well-known problem of vanishing and exploding gradients~\cite{bengiolongterm,pascanudifficulty}.
LSTM is a commonly used reparametrization that is specifically  designed to address the problem of vanishing and exploding gradients.
The LSTM recurrent transition is given by:
\begin{eqnarray}
...
\begin{figure}
\center
\includegraphics[width=6.7cm]{figures/tanh_grad.pdf}
\caption{Influence of pre-activation standard deviation on expected derivative of tanh nonlinearity}
\label{fig:tanh_grad}
\end{figure}

We dedicate this section to the straightforward but crucial observation that high-variance pre-activations tend to saturate sigmoid nonlinearities.
...
Batch normalization allows for easy control of the pre-activation variance,
...
% TODO: flesh out, show gradient propagation vs gamma plots
...
\caption{Convergence of population statistics to stationary distributions. Only a random subset of population statistics is shown.}
